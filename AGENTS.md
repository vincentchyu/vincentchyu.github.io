...
这是一个以 GitHub Pages 为主的个人网站仓库，核心内容分成四块：

1. 个人主页与旧站资源，主要在仓库根目录的 `index.html` 和 `web/before/`。
...
3. 照片管理后台，主要在 `web/admin/`，由 Go 后端提供 API。
4. SonicLens 子站，主要在 `web/sonic-lens/`，使用 Cloudflare Pages Functions。
...
- `web/before/`：旧站和历史资源，通常只在迁移或修复旧页面时改动。
