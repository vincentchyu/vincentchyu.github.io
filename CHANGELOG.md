...
- **山河足迹增量轨迹全自动闭环流水线 (`./run.sh synct`, `shell/script/sync_tracks_pipeline.py`)**：
  - **100% 复用 apple_health_export 核心能力**：基于其增量缓存机制（`.processed_cache.json`）与双层对齐引擎（高频瞬时线性插值 + `endDate` 区间均值覆盖 + 30~240 bpm 生理清洗），毫秒级提取最新未处理的体能训练路线，彻底杜绝逻辑重复实现。
  - **离线逆地理智能规范重命名**：利用站点现有的 `web/tracks/data/provinces.geojson`，基于纯标准库射线算法（零第三方依赖）在 1ms 内将经纬度映射至国家与省份，结合东八区时段与运动特征，自动重命名为 `{type}-{国家}-{省份}-{时段运动}-{YYYYMMDD}.gpx` 并存入 `~/.config/gpx/`。
  - **全链路一键闭环**：通过 `./run.sh synct` 自动探测 `~/Downloads/` 内的 `导出.zip`，流式提取、规范归档后自动触发 `cmd/update-tracks` 刷新全站，实现从手机导出数据包到网页端点亮足迹的“零人工干预”极致闭环。

### 缺陷修复 & 核心算法加固

- **山河足迹轨迹数据流水线核心计算与时空对齐缺陷全面修复 (`internal/track/`, `cmd/`)**：
  - **修复运动类型推断与速度过滤时序倒置漏洞**：在计算 `totalDistance` 前进行运动类型前置预判，确保未规范命名的 GPX 文件与规范化重命名前后测距严格一致，消除高程剖面 Profile 横轴拉伸失真。
  - **峰谷波动态迟滞高程滤波与海平面/低洼地支持**：升级 `SanitizeAndSmoothElevations` 为工业级 Peak-Valley 动态迟滞算法（1.5m 容差），并支持沿海海平面（0~1m）与负海拔（如吐鲁番艾丁湖 -154m）合法点位，杜绝平原与沿海正常高程被强行拉平。
  - **统一步调消除合并与串联中的千米级虚假爬升**：`MergeGPXTracksWithOptions` 与 `ConcatGPXTracks` 彻底废除未平滑的相邻点正落差直接累加，全链路统一步调接入 `SanitizeAndSmoothElevations`，消除了高达数千米的虚假累计爬升。
  - **修复首尾串联跨天营地与机动车转场逻辑**：纠正 `ConcatGPXTracks` 中 `||` 导致的短时间长距离机动车转移被计入徒步里程的逻辑漏洞，准确剔除高速转场（>35km/h 且 >5km）与夜间营地大位移。
  - **修复照片匹配时区错位与 20km 误吸漏洞**：照片时间与轨迹时间统一在东八区（CST）进行比对，消除跨午夜同日判定失效；将照片沿途空间容差收紧为 1km，且仅在活动前后 20 分钟窗口内匹配，杜绝将数小时之外同城生活区无关照片误关联入轨迹。
  - **心率插值双指针边界修复与盲区填充时序单调保障**：修复 `nextHR` 在轨迹起步早于心率记录时的索引偏移；副轨迹盲区候选点注入前执行全局与局部时序排序，杜绝时间倒流折线。
  - **孤儿清理灾难性误删防护与浮点累计舍入**：`update-tracks` 在扫描轨迹规模远小于现有分片时自动阻止全量清理，防止局部单文件测试误删线上历史分片；统计指标累加执行保留 2 位小数舍入，消除了浮点脏数据。
  - **BBox 脏点过滤与 Override 关键字容错**：`ParseQuickSummary` 增加 `(0,0)` 脏点过滤防包围盒膨胀；`GetOverride` 增加关键字模糊匹配，防止规范重命名后手动校准项静默失效。
  - **点位时间戳单调性保序与时空脏点清洗**：在解析 GPX 原始点时加入经纬度范围与 `(0,0)` 脏点过滤，并引入 `slices.SortStableFunc` 时间戳稳定排序与单调性兜底保护，消除车载/GPS 芯片在弱信号或隧道重新锁星时偶发的微小时间倒流（逆序），保障距离累加、速度推断、动画回放及照片时空插值的绝对单调性。

## 2026-08-27

### 新功能 & 界面体验优化

- **山河足迹数据流水线孤儿分片自动垃圾回收与紧凑压缩 (`cmd/update-tracks/`)**：
  - **默认自动清理无用历史 JSON 分片**：在生成最新 `manifest.json` 后，流水线自动对比并清理 `web/tracks/data/tracks/` 中不再被引用的孤儿分片文件，避免历史残留与存储膨胀。
  - **清单文件 Minified 紧凑极致压缩**：`manifest.json` 默认改用紧凑无空格压缩（从 379KB 缩减至 229KB，体积减少 40%），大幅加快首屏索引加载与浏览器 JSON 解析性能；同时提供 `--pretty` 选项支持格式化输出。
  - **支持显式参数控制**：新增 `--clean`（默认开启）与 `--clear` 参数，保障输出目录强一致性与绝对整洁。
