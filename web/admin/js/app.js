...
function renderPhotos() {
    ...
    if (useVirtualScroll) {
        ...
        photoGrid.innerHTML = filteredPhotos.map(photo => `
            <div class="photo-card ${photo.is_hidden ? 'hidden' : ''} ${selectedPhotos.has(photo.filename) ? 'selected' : ''}" 
                 data-filename="${photo.filename}">
                <input type="checkbox" class="photo-checkbox" data-filename="${photo.filename}" 
                ...
                <div class="photo-actions">
                    <button class="btn btn-small btn-preview-r2" data-filename="${photo.filename}">
                        预览 R2
                    </button>
                </div>
            </div>
...
function showDetail(filename) {
    ...
    document.getElementById('detailSize').value = `${currentPhoto.width}×${currentPhoto.height}`;
    document.getElementById('detailAlt').value = currentPhoto.alt || '';
    document.getElementById('detailTags').value = (currentPhoto.Subject || []).join(', ');
    document.getElementById('detailIsHidden').checked = currentPhoto.is_hidden;
    document.getElementById('detailImage').src = `/api/images/${currentPhoto.year}/${currentPhoto.filename}`;
...
async function saveDetail() {
    ...
    const updates = {
        ...
        Subject: document.getElementById('detailTags').value
            .split(',')
            .map(t => t.trim())
            .filter(t => t)
    };
...
async function deleteCurrentPhoto() {
    ...
    if (!confirm(`确定要删除照片 ${currentPhoto.filename} 吗？\n此操作将删除本地文件和 R2 上的文件，且无法恢复！`)) {
        return;
    }
    ...
    try {
        ...
        filteredPhotos = filteredPhotos.filter(p => p.filename !== currentPhoto.filename);
...
function updateStats() {
    ...
    document.getElementById('hiddenPhotos').textContent = allPhotos.filter(p => p.is_hidden).length;
...
function setupEventListeners() {
    ...
    // Batch actions
    document.getElementById('batchHideBtn').addEventListener('click', () => batchUpdate(true));
    document.getElementById('batchShowBtn').addEventListener('click', () => batchUpdate(false));
    
    // Filters
    document.getElementById('searchInput').addEventListener('input', filterPhotos);
...
async function handlePhotoUpload(e) {
    ...
    document.getElementById('uploadMessage').textContent = `准备上传 ${files.length} 张照片...`;
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        document.getElementById('uploadMessage').textContent = 
            `已上传 ${i + 1}/${files.length} (成功: ${uploaded}, 失败: ${failed})`;
    }
