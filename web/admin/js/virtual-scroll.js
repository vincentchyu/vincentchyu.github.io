...
class VirtualPhotoGrid {
    ...
    init() {
        ...
        const totalRows = Math.ceil(this.allPhotos.length / this.options.itemsPerRow);
        this.scrollContainer.style.height = `${totalRows * this.options.itemHeight}px`;
        
        // Set up scroll listener
        this.container.addEventListener('scroll', this.throttle(() => this.onScroll(), 100));
    ...
    render() {
        ...
        const startRow = Math.floor(scrollTop / this.options.itemHeight) - this.options.bufferRows;
        const endRow = Math.ceil((scrollTop + containerHeight) / this.options.itemHeight) + this.options.bufferRows;
        
        const visibleStart = Math.max(0, startRow * this.options.itemsPerRow);
        const visibleEnd = Math.min(this.allPhotos.length, endRow * this.options.itemsPerRow);
        ...
        // Calculate offset
        const offsetY = Math.floor(visibleStart / this.options.itemsPerRow) * this.options.itemHeight;
        
        // Render photos
        this.contentWrapper.innerHTML = visiblePhotos.map((photo, index) => {
    ...
    renderPhotoCard(photo, index) {
        ...
        return `
            <div class="photo-card ${photo.is_hidden ? 'hidden' : ''} ${isSelected ? 'selected' : ''}" 
    ...
    attachEventListeners() {
        ...
        // Checkbox changes
        this.contentWrapper.querySelectorAll('.photo-checkbox').forEach(checkbox => {
